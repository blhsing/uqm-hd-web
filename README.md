...
- 網頁不另顯示啟動畫面；開啟網址後即載入遊戲。瀏覽器首選語言為繁體中文
  （`zh-Hant`、`zh-TW`、`zh-HK` 或 `zh-MO`）時自動使用繁中，其他語言使用英文；
  右上角仍可在目前工作階段切換語言。
- 選單可直接以滑鼠或觸控點選。行動瀏覽器進入戰鬥後，左拇指區會顯示可拖曳的
  半透明轉向／推進搖桿，武器與特殊能力按鈕位於右拇指區；桌面瀏覽器維持原版
  鍵盤與遊戲控制器操作。
- 遊戲主選單不顯示「返回」。其他遊戲畫面的左上角「返回」會送出一次 `Esc`，
  以返回上一層、關閉目前畫面或執行該畫面原有的取消流程。
...
行動瀏覽器進入實際戰鬥後，網頁會自動顯示半透明觸控控制。用左拇指拖曳圓形搖桿：
向左／右拖曳會轉向，向上拖曳會推進，斜向上拖曳可同時推進及轉向。右下方的大按鈕
是主武器，旁邊較小的按鈕是特殊能力；多點觸控允許左右拇指同時操作。玩家一對應
方向鍵、右 `Ctrl` 與右 `Shift`，玩家二對應預設 ESDF 配置；若兩位本機玩家同時由
觸控操作，玩家二控制組會置於畫面上方。選單、艦隊格及船艦圖示可直接觸碰，不需先
移動虛擬游標。桌面瀏覽器不顯示觸控控制，完整保留原版鍵盤、滑鼠及遊戲控制器操作。
...
不改寫 UQM 封包。桌面端使用實體鍵盤或遊戲控制器，行動端使用網頁觸控控制操作遠端對戰；
外層「返回」按鈕會在主選單以外的畫面送出 `Esc`，主選單本身不顯示此按鈕。
