...
DoRestart (MENU_STATE *pMS)
{
	...
	if (!pMS->Initialized)
	{
		...
		pMS->flashContext = Flash_createOverlay (ScreenContext,
				NULL, NULL);
		// Traditional Chinese distribution change (2026-07-27): keep the
		// selected entry visibly yellow throughout the pulse.  The
		// original -3..3 range crossed through the unselected gray and spent
		// half the cycle subtracting the overlay, which made focus ambiguous.
		Flash_setMergeFactors (pMS->flashContext, 3, 6, 16);
