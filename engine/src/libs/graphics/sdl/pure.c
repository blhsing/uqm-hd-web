...
TFB_Pure_ConfigureVideo (int driver, int flags, int width, int height, int togglefullscreen, unsigned int resolutionFactor)  // JMS_GFX: Added resolutionFactor
{
	...
	if (SDL_WindowHandle == NULL)
	{
		SDL_WindowHandle = SDL_CreateWindow ("The Ur-Quan Masters",
				SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED,
				ScreenWidthActual, ScreenHeightActual, SDL_WINDOW_SHOWN);
	}
	else
	{
		SDL_SetWindowSize (SDL_WindowHandle, ScreenWidthActual,
				ScreenHeightActual);
	}
	if (SDL_WindowHandle != NULL)
	{
		SDL_SetWindowFullscreen (SDL_WindowHandle,
				(flags & TFB_GFXFLAGS_FULLSCREEN) ?
				SDL_WINDOW_FULLSCREEN_DESKTOP : 0);
		SDL_Video = SDL_GetWindowSurface (SDL_WindowHandle);
	}
#else
...
TFB_Pure_InitGraphics (int driver, int flags, int width, int height, unsigned int resolutionFactor)  // JMS_GFX: Added resolutionFactor
{
	...
	if (currentDriver == NULL)
		...
		currentDriver = VideoName;
#endif
	log_add (log_Info, "SDL driver used: %s", currentDriver);
...
#ifdef __EMSCRIPTEN__
	SDL_UpdateWindowSurfaceRects (SDL_WindowHandle, &updated, 1);
#else
...
TFB_Pure_ScreenLayer (SCREEN screen, Uint8 a, SDL_Rect *rect)
{
	if (SDL_Screens[screen] == backbuffer)
		return;
	TFB_SetSurfaceAlpha (SDL_Screens[screen], 1, a);
...
TFB_Pure_ColorLayer (Uint8 r, Uint8 g, Uint8 b, Uint8 a, SDL_Rect *rect)
{
	...
	TFB_SetSurfaceAlpha (fade_color_surface, 1, a);
