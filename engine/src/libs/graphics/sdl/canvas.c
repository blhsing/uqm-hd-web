...
TFB_DrawCanvas_Rect (RECT *rect, Color color, DrawMode mode, TFB_Canvas target)
{
	...
	{	// Standard SDL fillrect rendering
		if (fmt->Amask && TFB_SurfaceGetColorKey (dst, &colorKey))
		...
		SDL_Surface *dst, SDL_Rect *dst_r, DrawMode mode)
{
	...
	{	// Standard SDL surface-alpha blit
		...
		assert (!TFB_SurfaceHasAlphaMod (src));
		// Set surface alpha temporarily
		TFB_SetSurfaceAlpha (src, 1, mode.factor);
		SDL_BlitSurface (src, src_r, dst, dst_r);
		TFB_SetSurfaceAlpha (src, 0, 255);
	}
	else
	{	// Custom blit
		...
		int scaleMode, TFB_ColorMap *cmap, DrawMode mode, TFB_Canvas target)
{
	...
	if (NormalPal && cmap && img->colormap_version != cmap->version)
		TFB_SetPaletteColors (img->NormalImg, cmap->palette->colors, 0, 256);
	...
	if (scale != 0 && scale != GSCALE_IDENTITY)
	{
		if (scaleMode == TFB_SCALE_TRILINEAR && img->MipmapImg)
		{
			...
			if (TFB_DrawCanvas_IsPaletted (img->MipmapImg)
				...
				TFB_SetPaletteColors (img->MipmapImg, cmap->palette->colors, 0, 256);
		}
		...
		if (TFB_DrawCanvas_IsPaletted (surf))
		{
			...
			TFB_SetPaletteColors (surf, NormalPal->colors, 0, NormalPal->ncolors);
		}
...
TFB_DrawCanvas_Fill (SDL_Surface *src, Uint32 fillcolor, SDL_Surface *dst)
{
	...
	{	// colorkey-based fill
		Uint32 srckey;
		if (!TFB_SurfaceGetColorKey (src, &srckey))
		{
		...
		int scaleMode, Color color, DrawMode mode, TFB_Canvas target)
{
	...
	{	// set palette for fill-stamp
		...
		TFB_SetPaletteColors (surf, colors, 0, palette->ncolors);
...
TFB_DrawCanvas_SetPalette (TFB_Canvas target, Color palette[256])
{
	...
	TFB_SetPaletteColors (target, colors, 0, 256);
}
...
{
	Uint32 colorKey;
	if (TFB_SurfaceGetColorKey ((SDL_Surface *) canvas, &colorKey))
		return colorKey;
	return -1;
}
...
TFB_DrawCanvas_SetTransparentIndex (TFB_Canvas canvas, int index, BOOLEAN rleaccel)
{
	if (index >= 0)
	{
		...
		if (!TFB_DrawCanvas_IsPaletted (canvas))
		{
			...
			TFB_SetSurfaceAlpha (canvas, 0, 255); 
		}
	}
	else
	{
...
TFB_DrawCanvas_GetTransparentColor (TFB_Canvas canvas, Color *color)
{
	Uint32 colorKey;
	if (!TFB_DrawCanvas_IsPaletted (canvas)
			&& TFB_SurfaceGetColorKey ((SDL_Surface *) canvas, &colorKey))
	{
		Uint8 ur, ug, ub;
		...
		EXTENT* size, HOT_SPOT* dst_hs)
{
	...
	TFB_SurfaceGetColorKey (dst, &transparent);
	...
	else if (TFB_SurfaceGetColorKey (src, &ck0))
	...
	else if (TFB_SurfaceGetColorKey (mm, &ck1))
	{	// colorkey transparency
		mk1 = ~mmfmt->Amask;
		ck1 &= mk1;
	}
		...
		int scale, HOT_SPOT* src_hs, EXTENT* size, HOT_SPOT* dst_hs)
{
	...
	{
		Uint32 sourceKey;
		const BOOLEAN had_colorkey =
				TFB_SurfaceGetColorKey (src, &sourceKey) != 0;
		Uint8 key_r = 0, key_g = 0, key_b = 0;

		if (had_colorkey)
			SDL_GetRGB (sourceKey, srcfmt, &key_r, &key_g, &key_b);
	...
	else if (TFB_SurfaceGetColorKey (src, &ck))
	{	// colorkey transparency
		mk = ~srcfmt->Amask;
		ck &= mk;
	}
		...
		TFB_Canvas canvas2, POINT c2org, const RECT *interRect)
{
	...
	{	// colorkey transparency
		s1mask = ~surf1->format->Amask;
		s1key = 0;
		TFB_SurfaceGetColorKey (surf1, &s1key);
		s1key &= s1mask;
	}
	...
	{	// colorkey transparency
		s2mask = ~surf2->format->Amask;
		s2key = 0;
		TFB_SurfaceGetColorKey (surf2, &s2key);
		s2key &= s2mask;
	}
