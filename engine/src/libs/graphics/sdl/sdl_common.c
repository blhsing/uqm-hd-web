...
SDL_Window *SDL_WindowHandle = NULL;
#else
...
TFB_PreInit (void)
{
	...
	log_add (log_Info, "Using SDL version %d.%d.%d (compiled with "
			"%d.%d.%d)", linkedVersion.major,
			linkedVersion.minor, linkedVersion.patch,
...
#ifdef __EMSCRIPTEN__
	if (SDL_WindowHandle)
		SDL_SetWindowTitle (SDL_WindowHandle, caption);
#else
...
TFB_InitGraphics (int driver, int flags, int width, int height, unsigned int resolutionFactor, BOOLEAN forceAspectRatio) // JMS_GFX: added resolutionFactor
{
	...
	if (fs_height == 0)
	{
		...
		{
			curr_h = desktopMode.h;
			curr_w = desktopMode.w;
		}
#else
		...
		log_add (log_Debug, "fs_height %u, fs_width %u, current_w %u",
				fs_height, fs_width, curr_w);
	}
...
TFB_UninitGraphics (void)
{
	...
	if (SDL_WindowHandle)
	{
		SDL_DestroyWindow (SDL_WindowHandle);
		SDL_WindowHandle = NULL;
		SDL_Video = NULL;
	}
#endif
	SDL_Quit ();
...
TFB_DisplayFormatAlpha (SDL_Surface *surface)
{
	...
	newsurf = SDL_ConvertSurface (surface, dstfmt,
			TFB_SurfaceConversionFlags (surface));
	// SDL_SRCCOLORKEY and SDL_SRCALPHA cannot work at the same time,
	// so we need to disable one of them
	{
		Uint32 sourceKey;
		Uint32 convertedKey;
		if (TFB_SurfaceGetColorKey (surface, &sourceKey) && newsurf
				&& TFB_SurfaceGetColorKey (newsurf, &convertedKey)
				&& TFB_SurfaceHasAlphaMod (newsurf))
		TFB_SetSurfaceAlpha (newsurf, 0, 255);
	}
		...
		SDL_Rect *dstrect, int blend_numer, int blend_denom)
{
	...
	if (TFB_SurfaceGetColorKey (src, &colorkey))
	{
		has_colorkey = TRUE;
	}
	else
...
#ifdef __EMSCRIPTEN__
	(void) gamma;
	log_add (log_Info, "Browser display gamma is managed by the browser.");
#else
